...
if ( is_category() ) {
        ...
        if ($category instanceof WP_Term) {
                echo "<p class='mbold'>Category: <a href='" .esc_url($category_link) . "' target='_blank'>" . $category->name . "</p>";
        }
}
if ( is_tag() ) {
        $tag = get_queried_object();
        $tag_link = get_tag_link($current_tag->term_id);
        if ($tag instanceof WP_Term) {
                echo "<p class='mbold'>TOPIC: <a href='" .esc_url($tag_link) . "' target='_blank'>" . strtoupper($tag->name) . "</p>";
        }
}
if ( is_search() ) 
{
        echo "<p class='mbold'>You searched for: " . esc_html(get_search_query()) . "</p>";
}
?>
		...
		<div class="centre mt-0 mb-4">
			...
			<a href="/outdoors/feed" title="Keep up to date with the RSS Feed" class="ms-3 me-3 no-underline">
				<img src="<?php bloginfo('template_directory'); ?>/images/feed-icon32x32.png" alt="Feed" width="32" height="32" />
            		...
            		<a href="https://mstdn.social/@hill_wanderer/" target="_blank" title="Follow me on Mastodon" class="no-underline">
...
<div class="footer container mt-4">
	...
	<div class="container ps-0 pe-0">
		<hr>
		<footer>
			<p class="centre mt-4 pb-4">Copyright &copy; 1998-<?php date_default_timezone_set('Europe/London'); echo date("Y"); ?>, AssortedExplorations.com</p>
		</footer>
	</div>
</div>
<?php
