...
class Daimma_Menu_Elements {
	 ...
	 * The constructor.
	 *
	 * @param Daimma_Shared $shared An instance of the shared class.
	...
	public function display_list_table(
		...
		if ( ! is_null( $post_search_input ) && mb_strlen( trim( $post_search_input ) ) > 0 ) {
			...
			$total_items = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}daimma_$db_table $filter" );

			// Initialize the pagination class.
			require_once $this->shared->get( 'dir' ) . '/admin/inc/class-daimma-pagination.php';
			$pag = new Daimma_Pagination( $this->shared );
		...
		?>
			...
			$results = $wpdb->get_results(
				"SELECT * FROM {$wpdb->prefix}daimma_$db_table $filter ORDER BY $db_primary_key DESC $query_limit",
				ARRAY_A
			);
			// phpcs:enable
			...
			<?php if ( count( $results ) > 0 ) : ?>
				...
				<div class="daimma-crud-table">

					<!-- list of tables -->
					<table class="daimma-crud-table__daext-items">
						<thead>
						<tr>
							<th>
								<input type="checkbox" class="daimma-cb-select-all">
							</th>
							<?php
						...
						<?php foreach ( $results as $result ) : ?>
							<tr>
								<td><input type="checkbox"
											class="daimma-bulk-action-checkbox"
								...
								foreach ( $columns as $key => $column ) {
									...
									<td>
										...
										<?php if ( 0 === $key ) : ?>
											<a class="daimma-crud-table__item-name" href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
												<?php echo esc_html( stripslashes( $result[ $column['db_field'] ] ) ); ?>
											</a>
											<div class="daimma-crud-table__row-actions">
											<div class="daimma-crud-table__row-actions-single-action">
												<a href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Edit', 'import-markdown' ); ?></a>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="daimma-crud-table__row-actions-single-action">
												<form method="POST"
													...
													<?php wp_nonce_field( 'daimma_clone_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'daimma_clone_' . $this->menu_slug . '_nonce' ); ?>
													<button type="submit" name="clone_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Duplicate', 'import-markdown' ); ?></button>
												</form>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="daimma-crud-table__row-actions-single-action daimma-crud-table__row-actions-single-action-delete">
												<form method="POST"
														action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>"
														id="delete-item-<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
													<?php wp_nonce_field( 'daimma_delete_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'daimma_clone_' . $this->menu_slug . '_nonce' ); ?>
													<input type="hidden" name="delete_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>">
													<button type="submit" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Delete', 'import-markdown' ); ?></button>
												</form>
											</div>
										</div>
				...
				<!-- Bulk Actions -->
				<div class="daimma-crud-table-controls">

					<div class="daimma-crud-table-controls__bulk-actions">
						<form method="POST" action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $this->slug_plural ); ?>">
							<select name="bulk_action" id="bulk_action">
								<option value=""><?php esc_html_e( 'Bulk actions', 'import-markdown' ); ?></option>
								<option value="delete"><?php esc_html_e( 'Delete', 'import-markdown' ); ?></option>
							</select>
							<?php wp_nonce_field( 'daimma_bulk_action_' . $this->menu_slug, 'daimma_bulk_action_' . $this->menu_slug . '_nonce' ); ?>
							<input id="bulk-action-selected-items" type="hidden" name="bulk-action-selected-items" value="">
							<input id="daimma-submit-bulk-action" type="submit" class="button daimma-admin-page-button" value="<?php esc_html_e( 'Apply', 'import-markdown' ); ?>">
						</form>
					</div>

					<div class="daimma-crud-table-controls__pagination-container">
						<!-- Display the pagination -->
						<?php if ( $pag->total_items > 0 ) : ?>
							<div class="daimma-crud-table-controls__daext-tablenav">
									<span class="daimma-crud-table-controls__daext-displaying-num"><?php echo esc_html( $pag->total_items ); ?>&nbsp<?php esc_html_e( 'items', 'import-markdown' ); ?></span>
									<?php $pag->show(); ?>
							</div>
						<?php endif; ?>
					</div>
			...
			<?php else : ?>
				...
				$this->shared->save_dismissible_notice(
					array(
						'elements' => array(
							array(
								'type' => 'text',
								__( 'There are no results that match your filter.', 'import-markdown' ),
							),
						),
						'class'    => 'updated',
					)
				...
				if ( 0 === mb_strlen( trim( $filter ) ) ) {
					echo '<div class="daimma-crud-table__no-items-found-message">' . esc_html__( 'Nothing to show yet! Add some items by clicking the Add New button.', 'import-markdown' ) . '</div>';
				}
	...
	public function header_bar() {
		...
		if ( 'new' === $action ) {
			$page_title = __( 'Add New', 'import-markdown' ) . ' ' . $this->label_singular;
		} elseif ( null !== $edit_id ) {
			$page_title = __( 'Edit', 'import-markdown' ) . ' ' . $this->label_singular;
		} else {
		...
		<div class="daimma-header-bar">

			<div class="daimma-header-bar__left">
				<div class="daimma-header-bar__page-title"><?php echo esc_html( $page_title ); ?></div>
				<?php if ( 'list' === $action && 'crud' === $this->context && null === $edit_id ) : ?>
					<a href="<?php echo esc_url( get_admin_url() . 'admin.php?page=daimma-' . $this->slug_plural . '&action=new' ); ?>"
						class="daimma-button daimma-header-bar__add-new-button">
						<?php $this->shared->echo_icon_svg( 'plus' ); ?>
						<div class="daimma-header-bar__add-new-button-text"><?php esc_html_e( 'Add New', 'import-markdown' ); ?></div>
					</a>
				<?php endif; ?>
			</div>

			<div class="daimma-header-bar__right">
				<?php if ( 'new' === $action || null !== $edit_id ) : ?>
					<a href="#" onclick="document.getElementById('form1').submit()" class="daimma-btn daimma-btn-primary"><?php esc_html_e( 'Save Changes', 'import-markdown' ); ?></a>
				<?php endif; ?>
			</div>
	...
	public function new_item() {
		...
		<div class="daimma-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				wp_nonce_field( 'daimma_create_update_' . $this->menu_slug, 'daimma_create_update_' . $this->menu_slug . '_nonce' );
	...
	public function edit_item( $edit_id ) {
		...
		<div class="daimma-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				// Sanitize the field name.
				$db_table_name = $wpdb->prefix . 'daimma_' . $this->db_table;
	...
	public function section_header( $label, $section_id, $icon_id = null ) {
		...
		<div class="daimma-main-form__section-header group-trigger" data-trigger-target="<?php echo esc_attr( $section_id ); ?>">
			<div class="daimma-main-form__section-header-title">
				<?php $this->shared->echo_icon_svg( $icon_id ); ?>
				<div class="daimma-main-form__section-header-title-text"><?php echo esc_html( $label ); ?></div>
			</div>
			<div class="daimma-main-form__section-header-toggle">
	...
	public function input_field(
		...
		<div class="daimma-main-form__daext-form-field" valign="top" data-section-id="<?php echo esc_attr( $section_id ); ?>">
			<div><label for="title"><?php echo esc_html( $label ); ?><?php echo $required ? ' <span class="daimma-required">*</span>' : ''; ?></label></div>
			<div>
	...
	public function display_admin_toolbar() {
		...
		<div class="daimma-admin-toolbar">
			<div class="daimma-admin-toolbar__left-section">
				<div class="daimma-admin-toolbar__menu-items">
					...
					foreach ( $this->config['admin_toolbar']['items'] as $key => $item ) {
						...
						<a href="<?php echo esc_attr( $item['link_url'] ); ?>" class="daimma-admin-toolbar__menu-item <?php echo 'daimma-' . $this->menu_slug === $item['menu_slug'] ? 'is-active' : ''; ?>">
							<div class="daimma-admin-toolbar__menu-item-wrapper">
								<?php $this->shared->echo_icon_svg( $item['icon'] ); ?>
								<div class="daimma-admin-toolbar__menu-item-text"><?php echo esc_html( $item['link_text'] ); ?></div>
							</div>
						</a>
					...
					<div class="daimma-admin-toolbar__menu-item daimma-admin-toolbar__menu-item-more">
						<div class="daimma-admin-toolbar__menu-item-wrapper">
							<?php $this->shared->echo_icon_svg( 'grid-01' ); ?>
							<div class="daimma-admin-toolbar__menu-item-text"><?php esc_html_e( 'More', 'import-markdown' ); ?></div>
							<?php $this->shared->echo_icon_svg( 'chevron-down' ); ?>
						</div>
						<ul class="daimma-admin-toolbar__pop-sub-menu">
							...
							foreach ( $this->config['admin_toolbar']['more_items'] as $key => $more_item ) {
								...
								<li>
									<a href="<?php echo esc_attr( $more_item['link_url'] ); ?>" <?php echo 1 === intval( $more_item['pro_badge'], 10 ) ? 'target="_blank"' : ''; ?>>
										<?php echo '<div class="daimma-admin-toolbar__more-item-item-text">' . esc_html( $more_item['link_text'] ) . '</div>'; ?>
										<?php

										if ( true === isset( $more_item['pro_badge'] ) && $more_item['pro_badge'] ) {
											echo '<div class="daimma-admin-toolbar__pro-badge">' . esc_html__( 'PRO', 'import-markdown' ) . '</div>';
										}
			...
			<div class="daimma-admin-toolbar__right-section">
				<!-- Display the upgrade button in the Free version. -->
				<?php if ( constant( 'DAIMMA_EDITION' ) === 'FREE' ) : ?>
				<a href="https://daext.com/ultimate-markdown/" target="_blank" class="daimma-admin-toolbar__upgrade-button">
					<?php $this->shared->echo_icon_svg( 'diamond-01' ); ?>
					<div class="daimma-admin-toolbar__upgrade-button-text"><?php esc_html_e( 'Unlock Extra Features with UM Pro', 'import-markdown' ); ?></div>
				</a>
				<?php endif; ?>
				<a href="https://daext.com" target="_blank" class="daimma-admin-toolbar__daext-logo-container">
				<img class="daimma-admin-toolbar__daext-logo" src="<?php echo esc_url( $this->shared->get( 'url' ) . 'admin/assets/img/daext-logo.svg' ); ?>" alt="DAEXT" />
				</a>
			</div>
		</div>
	...
	public function display_pro_features() {
		...
		if ( constant( 'DAIMMA_EDITION' ) !== 'FREE' ) {
			return;
		}
		...
		<div class="daimma-admin-body">
			...
			<div class="daimma-pro-features">

				<div class="daimma-pro-features__wrapper">

					<div class="daimma-pro-features__left">
						<div class="daimma-pro-features__title">
							<div class="daimma-pro-features__title-text"><?php esc_html_e( 'Unlock Advanced Features with Ultimate Markdown Pro', 'import-markdown' ); ?></div>
							<div class="daimma-pro-features__pro-badge"><?php esc_html_e( 'PRO', 'import-markdown' ); ?></div>
						</div>
						<div class="daimma-pro-features__description">
							<?php
							esc_html_e(
						...
						<div class="daimma-pro-features__buttons-container">
							<a class="daimma-pro-features__button-1" href="https://daext.com/ultimate-markdown/" target="_blank">
								<div class="daimma-pro-features__button-text">
									<?php esc_html_e( 'Learn More', 'import-markdown' ); ?>
								</div>
								<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
							</a>
							<a class="daimma-pro-features__button-2" href="https://daext.com/ultimate-markdown/#pricing" target="_blank">
								<div class="daimma-pro-features__button-text">
									<?php esc_html_e( 'View Pricing & Upgrade', 'import-markdown' ); ?>
								</div>
								<?php
					...
					<div class="daimma-pro-features__right">
						...
						foreach ( $pro_features_data_a as $key => $pro_feature_data ) {
							...
							<div class="daimma-pro-features__single-feature">
								<div class="daimma-pro-features__single-feature-wrapper">
									<?php $this->shared->echo_icon_svg( $pro_feature_data['icon'] ); ?>
									<div class="daimma-pro-features__single-feature-name">
				...
				<div class="daimma-pro-features__footer-wrapper">
					<div class="daimma-pro-features__footer-wrapper-inner">
						<div class="daimma-pro-features__footer-wrapper-left">
							<?php esc_html_e( 'Built for WordPress creators by the DAEXT team', 'import-markdown' ); ?>
						</div>
						<a class="daimma-pro-features__footer-wrapper-right" href="https://daext.com/products/" target="_blank">
							<div class="daimma-pro-features__footer-wrapper-right-text">
								<?php esc_html_e( 'More Tools from DAEXT', 'import-markdown' ); ?>
							</div>
							<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
						</a>
					</div>
	...
	public function handle_duplicate() {
		...
		if ( ! is_null( $data['clone_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daimma_clone_' . $this->menu_slug . '_' . $data['clone_id'], 'daimma_clone_' . $this->menu_slug . '_nonce' );
			...
			$this->shared->save_dismissible_notice(
				array(
					'elements' => array(
						array(
							'type' => 'text',
							__( 'The item has been successfully duplicated.', 'import-markdown' ),
						),
					),
					'class'    => 'updated',
				)
	...
	public function handle_delete() {
		...
		if ( ! is_null( $data['delete_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daimma_delete_' . $this->menu_slug . '_' . $data['delete_id'], 'daimma_clone_' . $this->menu_slug . '_nonce' );
				...
				$this->shared->save_dismissible_notice(
					array(
						'elements' => array(
							array(
								'type' => 'text',
								__( 'The item has been successfully deleted.', 'import-markdown' ),
							),
						),
						'class'    => 'updated',
					)
	...
	public function handle_bulk_actions() {
		...
		if ( 'delete' === $bulk_action ) {
			...
			if ( ! is_null( $delete_id ) ) {
				...
				// Nonce verification.
				check_admin_referer( 'daimma_bulk_action_' . $this->menu_slug, 'daimma_bulk_action_' . $this->menu_slug . '_nonce' );
					...
					$this->shared->save_dismissible_notice(
						array(
							'elements' => array(
								array(
									'type' => 'text',
									$deleted_items_count . ' ' . __( 'items have been successfully deleted.', 'import-markdown' ),
								),
							),
							'class'    => 'updated',
						)
							...
							$this->shared->save_dismissible_notice(
								array(
									'elements' => array(
										array(
											'type' => 'text',
											__( 'The', 'import-markdown' ) . ' ' . strtolower( $this->label_plural ) . ' ' . __( "with the following IDs are used in one or more categories and can't be deleted:", 'import-markdown' ) . ' ' . implode( ', ', $delete_id_non_deletable ) . '.',
										),
									),
									'class'    => 'error',
								)
							...
							$this->shared->save_dismissible_notice(
								array(
									'elements' => array(
										array(
											'type' => 'text',
											__( 'The', 'import-markdown' ) . ' ' . strtolower( $this->label_plural ) . ' ' . __( "with the following IDs are used in one or more cookies and can't be deleted:", 'import-markdown' ) . ' ' . implode( ', ', $delete_id_non_deletable ) . '.',
										),
									),
									'class'    => 'error',
								)
	...
	public function print_form_fields_from_array( $sections ) {
		...
		foreach ( $sections as $section ) {
			...
			echo '<div class="daimma-main-form__daext-form-section" data-id="' . esc_attr( $section['section_id'] ) . '">';
			...
			echo '<div class="daimma-main-form__daext-form-section-body" data-section-id="' . esc_attr( $section['section_id'] ) . '">';
	...
	public function display_crud_menu_search_form( $post_search_input ) {
		...
		<form action="admin.php" method="GET" id="items-filter">
			...
			<div class="daimma-crud-table-search-form">

				<div id="daext-search-form">

					<input type="hidden" name="page" value="daimma-<?php echo esc_attr( $this->slug_plural ); ?>">
					<input class="daimma-crud-table-search-form__post-search-input" type="text" name="s"
							value="<?php echo null !== $post_search_input ? esc_attr( stripslashes( $post_search_input ) ) : ''; ?>" autocomplete="off" maxlength="255">
					<input class="button daimma-admin-page-button" type="submit" value="Search <?php echo esc_attr( $this->label_plural ); ?>">
