...
class Daimma_Pagination {
	 ...
	 * Constructor.
	 *
	 * @param Daimma_Shared $shared Instance of the shared class.
	 */
	...
	public function show() {
		...
		if ( $last_page > 1 ) {
			...
			echo '<div class="daimma-crud-table-controls__pagination-paging-text">' .
	...
	public function get_pagenum_link( $id ) {
		...
		if ( strpos( $this->target_page, '?' ) === false ) {
			return admin_url( $this->target_page . '?' . $this->parameter_name . '=' . $id . $search );
		} else {
			return admin_url( $this->target_page . '&' . $this->parameter_name . '=' . $id . $search );
		}
