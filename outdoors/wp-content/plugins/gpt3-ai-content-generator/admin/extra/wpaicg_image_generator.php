...
<style>
    ...
    .wpaicg-collapse-content .wpaicg-mb-5 .customSchedulerDiv{
        display: flex;
        align-items: center;
    }
    .wpaicg-mb-5{
...
// Set default behavior based on determined settings
if (!$wpaicg_show_dalle && $wpaicg_show_sd) {
    $wpaicg_action = 'stable-diffusion';
}
...
<div class="wrap fs-section wpaicg-image-generator<?php echo isset($wpaicg_image_shortcode) && $wpaicg_image_shortcode ? ' wpaicg-image-shortcode':''?>">
    ...
    <div class="wpaicg-image-generator-tabs">
        ...
        <a href="<?php echo esc_url($wpaicg_sd_url)?>" class="<?php echo $wpaicg_action == 'stable-diffusion' ? ' wpaicg-tab-active' : ''?>"><?php echo esc_html__('Replicate','gpt3-ai-content-generator')?></a>
        <?php
        }
        ?>
    </div>
    ...
    <h2 class="nav-tab-wrapper">
        ...
        \WPAICG\wpaicg_util_core()->wpaicg_tabs('wpaicg_image_generator',array(
            ...
            'stable-diffusion' => esc_html__('Replicate','gpt3-ai-content-generator'),
                    ...
                    <div class="wpaicg_grid_form_1">
                        ...
                        <div class="wpaicg-collapse wpaicg-collapse-active" style="<?php echo $wpaicg_show_setting ? '' : 'display:none'?>">
                            ...
                            <div class="wpaicg-collapse-content">
                                ...
                                <div class="wpaicg-mb-5">
                                    <?php
                                    echo '<label for="img_model" class="wpaicg-form-label">'.esc_html__('Model','gpt3-ai-content-generator').':</label>';
                                    echo '<select class="wpaicg-input" name="img_model" id="img_model">';
                                    // models: Dall-E 3 HD, Dall-E 3, Dall-E 2
                                    if(isset($_POST['img_model'])) {
                                        echo '<option value="dall-e-3-hd" ' . (sanitize_text_field($_POST['img_model']) == "dall-e-3-hd" ? 'selected' : '') . '>Dall-E 3 HD</option>';
                                        echo '<option value="dall-e-3" ' . (sanitize_text_field($_POST['img_model']) == "dall-e-3" ? 'selected' : '') . '>Dall-E 3</option>';
                                        echo '<option value="dall-e-2" ' . (sanitize_text_field($_POST['img_model']) == "dall-e-2" ? 'selected' : '') . '>Dall-E 2</option>';
                                    } else {
                                        echo '<option'.(isset($wpaicg_image_settings['img_model']) && $wpaicg_image_settings['img_model'] == 'dall-e-3-hd' ? ' selected':'').' value="dall-e-3-hd">Dall-E 3 HD</option>';
                                        echo '<option'.(!isset($wpaicg_image_settings['img_model']) || $wpaicg_image_settings['img_model'] == '' || $wpaicg_image_settings['img_model'] == 'dall-e-3' ? ' selected':'').' value="dall-e-3">Dall-E 3</option>';
                                        echo '<option'.(isset($wpaicg_image_settings['img_model']) && $wpaicg_image_settings['img_model'] == 'dall-e-2' ? ' selected':'').' value="dall-e-2">Dall-E 2</option>';
                                    }
                                    echo '</select>';
                                    ?>
                                </div>
                                <div class="wpaicg-mb-5">
                                    ...
                                    if(isset($_POST['img_size'])) {
                                        ...
                                        echo '<option value="1792x1024" ' . (sanitize_text_field($_POST['img_size']) == "1792x1024" ? 'selected' : '') . '>1792x1024</option>';
                                        echo '<option value="1024x1792" ' . (sanitize_text_field($_POST['img_size']) == "1024x1792" ? 'selected' : '') . '>1024x1792</option>';
                                        ...
                                        echo '<option'.(isset($wpaicg_image_settings['img_size']) && $wpaicg_image_settings['img_size'] == '1792x1024' ? ' selected':'').' value="1792x1024">1792x1024</option>';
                                        echo '<option'.(isset($wpaicg_image_settings['img_size']) && $wpaicg_image_settings['img_size'] == '1024x1792' ? ' selected':'').' value="1024x1792">1024x1792</option>';
                                    }
                                    echo '</select>';
                                    ?>
                                </div>
                                <div class="wpaicg-mb-5">
                                    <?php
                                    echo '<label for="img_type" class="wpaicg-form-label">' . esc_html__('Type', 'gpt3-ai-content-generator') . ':</label>';
                                    echo '<select class="wpaicg-input" name="img_type" id="img_type">';
                                    if (isset($_POST['img_type'])) {
                                        $selected_img_type = sanitize_text_field($_POST['img_type']);
                                    } else {
                                        $selected_img_type = isset($wpaicg_image_settings['img_type']) ? $wpaicg_image_settings['img_type'] : 'vivid';
                                    }
                                    echo '<option value="vivid"' . ($selected_img_type == "vivid" ? ' selected' : '') . '>Vivid</option>';
                                    echo '<option value="natural"' . ($selected_img_type == "natural" ? ' selected' : '') . '>Natural</option>';
                                    echo '</select>';
                                    ?>
                                </div>
                                ...
                                <div class="wpaicg-mb-5">
                                    ...
                                    $max_images = ($wpaicg_provider == 'Azure') ? 5 : 10;
                                ...
                                ?>
                                    ...
                                    <div class="wpaicg-mb-5">
                                        <label class="wpaicg-form-label"><?php echo esc_html__('Inference Steps','gpt3-ai-content-generator')?></label>
                                    ...
                                    <div class="wpaicg-mb-5">
                                        ...
                                        <select name="scheduler" id="schedulerDropdown">
                                        <?php
                                            $schedulers = array('DDIM', 'K_EULER', 'DPMSolverMultistep', 'K_EULER_ANCESTRAL', 'PNDM', 'KLMS', 'Custom');
                                            $storedScheduler = isset($wpaicg_image_settings['scheduler']) ? $wpaicg_image_settings['scheduler'] : 'DPMSolverMultistep';

                                            foreach($schedulers as $scheduler) {
                                                $isSelected = false;

                                                if ($scheduler === 'Custom' && !in_array($storedScheduler, $schedulers)) {
                                                    $isSelected = true;
                                                } elseif ($scheduler == $storedScheduler) {
                                                    $isSelected = true;
                                                }

                                                echo '<option' . ($isSelected ? ' selected' : '') . ' value="' . esc_html($scheduler) . '">' . esc_html($scheduler) . '</option>';
                                            }
                                            ?>

                                    </select>
                                    </div>
                                    <div class="customSchedulerDiv" style="<?php echo isset($wpaicg_image_settings['scheduler']) && !in_array($wpaicg_image_settings['scheduler'], $schedulers) ? 'display:block' : 'display:none'; ?>">
                                        <label class="wpaicg-form-label"><?php echo esc_html__('Custom Scheduler:', 'gpt3-ai-content-generator'); ?></label>
                                        <input class="wpaicg_input custom_scheduler" type="text" id="customSchedulerInput" name="customScheduler" value="<?php echo isset($wpaicg_image_settings['scheduler']) && !in_array($wpaicg_image_settings['scheduler'], $schedulers) ? esc_attr($wpaicg_image_settings['scheduler']) : ''; ?>">
                                    </div>
                                <?php
                                endif;
                                ?>
...
?>
<script>
    let schedulerDropdown = document.getElementById("schedulerDropdown");
    if (schedulerDropdown) {
        schedulerDropdown.addEventListener("change", function() {
            const customSchedulerDiv = document.querySelector(".customSchedulerDiv");
            if (this.value === "Custom") {
                customSchedulerDiv.style.display = "block";
            } else {
                customSchedulerDiv.style.display = "none";
            }
        });
    }
